...
function populateDefaults(info){
  ...
  document.querySelector(".crop .holder").addEventListener( 'click', function ( event ) {
...
runFunctions.crop_image = function(info){
  ...
  el.src = `/editable/${dbinfo.defaults.newimagesfolder}/${info.img}${queryString}`;
}
  ...
  if(info.success != null && info.success != undefined){
    ...
    setTimeout(() => {
      ...
      info.success.forEach(function(link){
        ...
        let html = `<div class="img-holder"><div class="number">cover ${num}</div><img id="${num}" src="/editable/${dbinfo.defaults.newimagesfolder}/${url}"></div>`;
...
function update_current_images(imgs = []){
  if(imgs.length > 0){
    ...
    imgs.forEach(function(img,indx){
      ...
      let timestamp = new Date().getTime();
      let queryString = "?t=" + timestamp;
      html += `<div class="img-holder"><div class="number">cover ${inx}</div><img id="${inx}" src="/editable/${idbinfo.defaults.newimagesfolder}/${img+queryString}"></div>`;
...
Object.entries(document.getElementsByTagName("button")).map(( object ) => {
  object[1].addEventListener("click", () => {
    ...
    if(action == 'code_toggle'){
      ...
      if(document.querySelector(code).classList.contains('hide')){
        show(code);
...
function outputFullTemplate(){
  ...
  function () {
   ...
   let encodedStrLive = removeForLive.replace(/[\u00A0-\u9999<>\&]/gim, function(i) {
       return '&#'+i.charCodeAt(0)+';';
    });
